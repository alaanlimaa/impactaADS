...
qtdCanais = int(input('Digite a quantidade de canais: '))
lista = []
for c in range(1, qtdCanais + 1):
    num = input(f'Dados do canal {c}: ')
    if ' ' in num:
        numE = num.replace(' ', '-')
        ...
        lista.append(num)
premium = float(input('Monetização Premium: R$ '))
notPremium = float(input('Monetização NÃO Premium: R$ '))
txt('BÔNUS')
for s in lista:
    insc = int(s[1])
    mont = float(s[2])
